...
function createSquare(gl) 
{
    ...
    var square = {buffer:squareVertexBuffer, vertSize:3, nVerts:4, primtype:gl.TRIANGLE_STRIP};
    return square;
}
...
function createTriangle(gl)
{
    ...
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);
    var triangle = {buffer:triangleVertexBuffer, vertSize:3, nVerts:3, primtype:gl.TRIANGLE_STRIP};
...
function createSphere(gl, radius)
{
    ...
    let verts=[0,0,0]
    for(var i=25;i<340;i++){
        var rad=i*(Math.PI/180)
        verts.push(radius*Math.cos(rad))
        verts.push(radius*Math.sin(rad))
        verts.push(0)
    }
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);
    var sphere = {buffer:sphereVertexBuffer, vertSize:3, nVerts:312, primtype:gl.TRIANGLE_FAN};
    return sphere;
}        